...
# -------------------------------
# Setup Kiosk Mode in .bashrc
# -------------------------------
bashrc_guard_line='if [[ -z "$SSH_CONNECTION" && "$(tty)" == "/dev/tty1" ]]; then'
bashrc_end_guard_line='fi'
comment_marker="######## KIOSK MODE ########"
bashrc_kiosk_logic=$(cat <<EOF
  echo -e "\033[0;32mWaiting for network...\033[0m"
  MAX_RETRIES=30
  RETRY_DELAY=2
  COUNT=0
  while ! ip route | grep default >/dev/null 2>&1; do
      COUNT=\$((COUNT + 1))
      if [ "\$COUNT" -ge "\$MAX_RETRIES" ]; then
          echo -e "\033[0;33mGave up waiting for network. Continuing anyway...\033[0m"
          break
      fi
      echo "No network yet... retry \$COUNT/\$MAX_RETRIES"
      sleep "\$RETRY_DELAY"
  done

  echo -e "\033[0;32mStarting kiosk...\033[0m"
  xinit /home/pi/kiosk -- vt\$(fgconsole)
EOF
)

# Add kiosk logic to .bashrc if not already present
if ! grep -Fq "$comment_marker" /home/pi/.bashrc; then
    echo -e "\033[0;32mAdding kiosk mode startup logic to .bashrc...\033[0m"
    {
        echo -e "\n$comment_marker"
        echo "$bashrc_guard_line"
        echo "$bashrc_kiosk_logic"
        echo "$bashrc_end_guard_line"
    } >> /home/pi/.bashrc
else
    echo -e "\033[0;32mKiosk logic already present in .bashrc.\033[0m"
fi
...
openbox --config-file /home/pi/.config/openbox/rc.xml &
...
# -------------------------------
# Configure Openbox (Fullscreen, No Borders)
# -------------------------------
echo -e "\n${unbold_orange}Creating Openbox config for Midori fullscreen...${unbold}"

mkdir -p /home/pi/.config/openbox
cat << EOF > /home/pi/.config/openbox/rc.xml
...
chown -R pi:pi /home/pi/.config /home/pi/kiosk

echo -e "${unbold_green}Openbox config created.${unbold}"
